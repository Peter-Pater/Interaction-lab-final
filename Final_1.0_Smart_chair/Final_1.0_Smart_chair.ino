//partly referred to Serial Call and Response Example
//with the original comments attached
/*
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    // thirdSensor = map(digitalRead(2), 0, 1, 0, 1);
    thirdSensor = analogRead(A2);
    // send sensor values:
    // converting sensor1 data
    if (firstSensor <= 16)
    {
      firstSensor = 16;
    }
    a = (2076.0 / (firstSensor - 11.0));
    //concerting sensor2 data
    if (secondSensor <= 22)
    {
      secondSensor = 22;
    }
    b = (4800.0 / (secondSensor - 20.0));
    ...
    //alarm system starts when the user sits
    if (thirdSensor > 20 && inByte == 'A') {
      //no vibrating when posture is "straight enough"
      if (b - a <= 10)
      {
        digitalWrite(9, LOW);
        digitalWrite(10, LOW);
      ...
      if (b - a > 10)
      {
        ...
        digitalWrite(9, LOW);
        delay(500);
        //alarm goes off when wrong posture for over 10 secs
        if (millis() - t1 > 10000)
        {
      ...
      //back to right posture, reset the variables
      digitalWrite(9, LOW);
      digitalWrite(10, LOW);
      t1 = millis();
    }
...
//refered to example (example,SerialCallResponse)
